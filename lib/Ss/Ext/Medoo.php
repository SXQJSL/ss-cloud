...
class Medoo
{
	...
	public function selectpdo($table, $join, $columns = null, $where = null)
	{
	    $query = $this->query($this->select_context($table, $join, $columns, $where));
	
	    return $query ? $query->fetchAll(
	        (is_string($columns) && $columns != '*') ? PDO::FETCH_COLUMN : PDO::FETCH_NUM
	    ) : false;
	}
	...
	public function queryown($querytext){
           $mysqli = new mysqli($this->server, $this->username, $this->password, $this->database_name,$this->port);
